...
function toggleSR() {
  return {
    listen: (cb: (arg0: any) => any) => socket.on('toggle_sr', (data: any) => cb(data)),
...
function callbackSR() {
  return {
    listen: (cb: (arg0: any) => any) => socket.on('callback_sr', (value: any) => cb(value)),
...
function changeLanguageSR() {
  return {
    listen: (cb: (arg0: any) => any) => socket.on('change_language_sr', (value: any) => cb(value)),
...
function checkConnectionSR() {
  return {
    listen: (cb: (arg0: any) => any) => socket.on('check_connection_sr', (value: any) => cb(value)),
